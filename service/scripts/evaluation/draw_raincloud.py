...
def draw_raincloud(
    ...
    The plot will facet by phase on the x-axis and show distributions per system.
    """
    matplotlib.use('Agg')
    ...
    if handles and labels:
        ...
        if unique:
            ax.legend(
                [u[0] for u in unique], [u[1] for u in unique], ncol=len(unique), 
    ...
    out_path = output_path or os.path.join(os.path.dirname(__file__), 'result_learning_outcome.pdf')
...
def main():
    parser = argparse.ArgumentParser(description='Draw a raincloud plot from learning_outcome.xlsx')
    ...
    parser.add_argument('--out', type=str, default=None, help='Output path (default: result_raincloud.pdf next to this script)')
    ...
    print(f"Saved raincloud plot to: {out_path}")

    # Print a textual summary similar to draw_learning_outcome.py
    # Try to produce Pre-test, Post-test, Knowledge gains per system
    import pandas as pd
    summary_rows = []
